...
function preventInspection() {
  ...
  // Detect if DevTools is open (heuristic based on window size)
  let devToolsOpen = false;
  const threshold = 160; // Pixel threshold for DevTools detection
  const checkDevTools = () => {
    ...
    if (widthThreshold || heightThreshold) {
      if (!devToolsOpen) {
        devToolsOpen = true;
        alert(
          "Developer Tools detected. Please close them to continue using the quiz."
        );
...
function initApp() {
  ...
  try {
    preventInspection(); // Initialize anti-inspection measures
    const loggedInUser = localStorage.getItem("currentUser");
    if (loggedInUser) {
      currentUser = JSON.parse(loggedInUser);
      const sessionToken = localStorage.getItem("sessionToken");
      const activeSessions =
        JSON.parse(localStorage.getItem("activeSessions")) || {};
      ...
      if (
        subData[currentUser.username] &&
        subData[currentUser.username].end > now
      ) {
        isSubscribed = true;
...
function showStartScreen() {
  ...
  try {
    ...
    console.error("AdMob ad failed to load:", e);
  }
}
...
async function registerUser() {
  ...
  try {
    ...
    if (!/^[A-Za-z][A-Za-z0-9_]*$/.test(username)) {
      showMessage("register-error", "Username must start with a letter and contain only letters, numbers, or underscores.");
      return;
    }
    if (password.length < 6) {
      showMessage("register-error", "Password must be at least 6 characters long.");
      return;
    }
    ...
    if (response.ok) {
      showMessage("register-error", "Registration successful", true);
    ...
    showMessage("register-error", "Network or server error. Please try again.");
  }
}
...
async function loginUser() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      // Check subscription (localStorage for now)
      ...
      showMessage("login-error", data.message || "Invalid username or password.");
    }
  } catch (error) {
    console.error("Login error:", error);
    showMessage("login-error", "Network or server error. Please try again.");
  }
}

// Placeholder for generateSessionToken (ensure it exists in your script.js)
function generateSessionToken() {
  return Math.random().toString(36).substr(2) + Date.now().toString(36);
}
...
function submitAccessCode() {
  ...
  try {
    ...
    if (!usernameCodes[firstLetter]) {
      showMessage(
        "subscription-error",
        "Invalid username: First letter must be A-Z."
      );
      return;
    }
    ...
    if (code !== expectedCode) {
      showMessage(
        "subscription-error",
        `Invalid access code for ${subscriptionMonths}-month plan.`
      );
      return;
    }
    ...
    showMessage(
      "subscription-error",
      `Subscription activated for ${subscriptionMonths} months!`,
      true
    );
    showStartScreen();
  } catch (error) {
    ...
    showMessage(
      "subscription-error",
      "Failed to process code. Please try again."
    );
  }
}
...
function checkSession() {
  if (currentUser) {
    ...
    if (
      ...
      logoutUser();
      alert(
        "Your session has been terminated due to a login on another device."
      );
    }
  }
}
...
function startQuiz() {
  ...
  try {
    ...
    if (availableQuestions === 0) {
      showMessage(
        "start-error",
        `No questions available for ${currentCourse}.`
      );
      return;
    }
    ...
    if (isSubscribed && ![25, 50, 100].includes(questionCount)) {
      ...
      showMessage(
        "start-error",
        "Free users can only select 15 questions (or available questions if fewer)."
      );
      return;
    }
    ...
    // Adjust time based on number of questions
    timeLeft =
      maxQuestions <= 15
        ? 15 * 60
        : maxQuestions <= 25
      ...
      buttons.forEach((button) => {
        if (button.textContent === answered[currentQuestion].selected) {
          button.classList.add(
            answered[currentQuestion].correct ? "correct" : "incorrect"
          );
        }
        if (
...
function endQuiz() {
  ...
  document.getElementById(
    "score"
  ).textContent = `${score} out of ${selectedQuestions.length}`;
